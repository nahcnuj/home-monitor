...
- `domains` — クエリ先ドメイン一覧
- `lookup_timeout_sec` — `nslookup` の **1 回の待ち上限**（デフォルト **60 秒**、`-timeout=60 -retry=0`）。無応答時は最大この秒数で `dns_timeout`。記録 ms はプロセス開始〜終了の壁時計
- `job_timeout_sec` — ジョブ打ち切り（デフォルト **70 秒**）。`nslookup` がそれ以上ハングしたとき `job_timeout`
