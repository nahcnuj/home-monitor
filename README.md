...
表示範囲は **画面幅いっぱいに何時間分を見せるか（ズーム）** です。グラフの時間軸は `data_cutoff_ts` 以降にデータがある限り過去へ伸び、横スクロールで遡れます。いずれのレンジでも **ファーストビューは右端（直近）** です。履歴が選択範囲より長いときだけスクロールし、Y 軸（ms）と凡例は固定、プロットだけが横に動きます。
