...
表示範囲は **画面幅いっぱいに何時間分を見せるか（ズーム）** です。チャート本体は常にその期間だけを描画し、履歴が長いときは下のスクロールバーで過去へパンします（巨大キャンバスは使いません）。いずれのレンジでも **ファーストビューは直近** です。測定数・平均・P95・最大・Uptime・エラー内訳も、いま画面に出ている時間窓だけを対象にします（パンするとその区間に合わせて更新されます）。
